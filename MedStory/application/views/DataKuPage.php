...
<html>
    <head>
		...
		<style>
			...
			* {
			...
			.col-md-9 .container .row .col-md-1 .container{
				width: 30px; 
				height:30px;
				border-radius:100%;
			}
    ...
    <body>
		...
		<div class="container bg-white mb-2 mt-2 p-3" style="border-radius: 10px;">
			<h5>Statistik Kebutuhan Kalori</h5>	
			<div class='container'>
				<div class='row'>
					<div class='col-md-6'>   
						<!--Chart.-->
						<h4 class='text-secondary'>Kebutuhan harian: </h4>
							<?php
								$i = 0; //Count item.
								...
								if($i != 0){
									...
									echo "<br><div class='container mt-1 mb-1'>
										<p id='msgInfo'>Maaf, Kebutuhan harian Anda masih kosong</p>
										<img src='http://localhost/MedStory/assets/icon/Empty.gif' alt='Empty.png' id='msgImage'>
									</div>";
								}
							?>
						<br>

						<h5 class='text-secondary'>Rata-rata</h5>
						<a>Kebutuhan harian: 
							<?php
								$hari = 0; //Count days.
								$i = 0; //Count item.
								...
								if($i != 0){
									$round = $hari/$i;
									echo number_format($round, 2); //To 2 digit decimal.
								} else {
									echo "- ";
								}	
							?> 
						...
						<p style='font-size:14px;'>Terpenuhi harian: 
							<?php
								$asupan = 0; //Total calorie.
								$hari = 0; //Count days.
								$i = 0; //Count item.
								$j = 0; //Count item.
								foreach($dataUser as $user){
								...
								if($j == 0){
									echo "<br><div class='container' style='margin-top:1%; margin-bottom:2%;'>
										<p id='msgInfo'>Maaf, Anda belum menambahkan asupan apapun</p>
									</div>";
								}
							?> 
						cal</p> 							
					...
					<!--Top chart.-->
					<div class='col-md-6'>
						<p style='font-size:16px; font-weight:500; color:#808080;'>Asupan terfavorit: </p> 
						<div class='container-fluid'>
							<?php
								$i = 1; //Count item.
								$x = 5; //Set max item on top chart.
								foreach($topAsupan as $top){
									foreach($dataAsupan as $asupan){
										if(($i <= $x) && ($top['id_asupan'] == $asupan['id_asupan'])){
											echo"<div class='card p-1 rounded border-0 mb-1' style='height:75px; box-shadow: #d4d4d4 0px 4px 12px;'>
												<div class='row'>
													<div class='col-sm-1' id='posFrame'>";
														if($i == 1){
													...
													<div class='col-sm-7' >
														<p style='color:#22A7F0;'>".$asupan['nama']."</p>
														<p class='text-secondary'>".$asupan['kalori']." cal / ".$asupan['ukuran']."</p>
													</div>
													<div class='col-sm-1 mt-4' style='margin-left:-30px;'>
														<h4>".$top['jumlah']."x </h4>
													</div>
												</div>
											</div>";
								...
								if($i == 1){
									echo "<div class='container' style='margin-top:1%; margin-bottom:2%;'>
										<p id='msgInfo'>Maaf, Asupan harian Anda masih kosong</p>
										<img src='http://localhost/MedStory/assets/icon/Empty.gif' alt='Empty.png' id='msgImage'>
									</div>";
								}
							?>
			...
			<div class="modal-dialog modal-lg" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="row">
							<div class="col-md-6">
								<img src="" style='width:340px; height:200px; border-radius:4px;' id='imageAsupan'>
							</div>
							<div class="col-md-5">
										...
										<div class="tab-pane fade shadow rounded bg-white show active p-3" id="v-pills-sayuran" role="tabpanel" aria-labelledby="v-pills-home-tab"
											...
											foreach($dataAsupan as $asupan){
												if($asupan['kategori'] == 'Sayuran'){
													echo"<div class='card p-1 rounded mb-1' style='border:none; height:75px; box-shadow: #d4d4d4 0px 4px 12px;";
					...
					<div class='row'>
						<div class='col-md-9'>
							<div class='container' style='float:left; position:relative;'>
								<div class='row' style='width:100%;'>
									<div class='col-md-1'>
										<div class='container' style='background:#29bd04;'></div>
									</div>
									<div class='col-md-2'>
										<a style='font-size:14px;'>Rendah</a>
									</div>
									<div class='col-md-1'>
										<div class='container' style='background:#de6000;'></div>
									</div>
									<div class='col-md-2'>
										<a style='font-size:14px;'>Sedang</a>
									</div>
									<div class='col-md-1'>
										<div class='container' style='background:#df4759;'></div>
									</div>
									<div class='col-md-2'>
		...
		<div class="modal fade" id="kaloriModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
			<div class="modal-dialog modal-lg" role="document">
				...
				<div class="modal-header">
					...
					<div class="col-sm">
						<input required class='form-control' name='date2' type='date' id='date2'></input>
					</div>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
					</button>
				...
				<div class="modal-body" >
					...
					<div class='container' style='margin-left:30%;'>
						<div class='row'>
							<input class="form-control" name='calTotal'></input>
							<button class='btn btn-success ml-1' type='submit'>Tambah</button>
						</div>
					</div>
				</div>
				<div class="modal-footer">
		...
		<?php 
			if(isset($error_tambahCalHarian)) { echo"
		...
		<script type="text/javascript">
			...
			$('#calendar').fullCalendar({
				...
				//Day click, insert daily calorie and asupan.
